...
import { Clock, Map as MapIcon, MapPin, Navigation } from "lucide-react";
...
const MetroMap: React.FC = () => {
  ...
  const [busStationDetail, setBusStationDetail] =
    useState<BusStationDetail | null>(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [stationsResponse, routesResponse, busStationsResponse] =
          await Promise.all([
            apiGetStations(),
            apiGetRoutes(),
            apiGetBusStation(),
        ...
        if (stationsResponse?.data) {
          ...
          const sortedStations = stationsResponse.data.sort(
        ...
        <div className="space-y-8">
          ...
          <div className="bg-white rounded-lg shadow-lg p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {routes.map((route) => {
                const routeStations = stations.filter(
              ...
              <div className="bg-gray-50 rounded-lg p-6">
                ...
                {schedulesLoading ? (
                  ...
                  <div className="space-y-4">
                    ...
                    {/* Backward Direction */}
                    {schedules.filter((s) => s.direction === "backward")
                      .length > 0 && (
                      <div>
                        <h3 className="text-lg font-medium text-gray-900 mb-3 flex items-center">
                  ...
                  <div className="bg-white rounded-lg border border-amber-200 p-6 mb-6">
                    ...
                    <div className="space-y-4">
                      {busStationDetail.routes.map((route) => (
                        <div
                          key={route.id}
                          className="bg-gradient-to-r from-amber-50 to-amber-100 border border-amber-200 rounded-xl p-6 shadow-sm hover:shadow-md transition-shadow duration-200"
                        >
                          {/* Route Header */}
                          <div className="flex justify-between items-start mb-4">
                            <div className="flex-1">
                              <h4
                                className="font-bold text-xl text-amber-900 mb-2 truncate"
                                title={route.name}
                              >
                                {route.name}
                              </h4>
                              <div className="flex items-center gap-3">
                                <span className="bg-amber-200 px-3 py-1 rounded-full font-medium text-amber-800">
                                  Route #{route.route_num}
                                </span>
                                <span
                                  className="text-amber-700 font-medium truncate"
